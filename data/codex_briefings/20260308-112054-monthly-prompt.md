...
请先完成组合诊断和资产配置判断，再综合第三到第五部分需要表达的调研结论，写第二部分“配置与操作建议”，最后把第二部分压缩成第一部分“本月直接操作建议”。
第一部分必须是 3 到 5 条直接、可执行、偏长期配置的动作句，每行一条，尽量简短，不展开长论证。
...
5. 第一部分是“本月直接操作建议”，第二部分是“配置与操作建议”；真正的配置建议只能出现在前两部分，后面三部分主要是帮助用户理解近一个月动态
...
- 不要展示 URL，不要展示“来源列表”
- 不要写免责声明，不要写空泛套话
- 第一部分必须是 3 到 5 条非常直接的动作建议，每行一条，可以用 `- ` 开头；要像给长期投资者看的执行摘要，而不是长段落分析
- 第一部分只写“接下来怎么做”，少解释原因，不要展开成长篇论证
...
===== MONTHLY MATERIAL PACKET JSON START =====
{
  ...
  "portfolio_snapshot": {
    "generated_at": "2026-03-08T11:20:11",
    "portfolio": {
      "position_count": 4,
