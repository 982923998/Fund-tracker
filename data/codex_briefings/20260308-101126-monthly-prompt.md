...
请直接依据下面给出的项目私有 skill、行业研究参考 skill 和材料包，生成最终月报。
请严格遵守 skill 的写作规则，并只返回符合 schema 的 JSON。
请先综合第二到第四部分需要表达的调研结论，再回到第一部分写配置与操作建议。
第一部分必须体现本月宏观、行业和大类资产研究结果对当前配置的含义，不能只重复仓位现状。
特别注意：配置建议只能出现在第一部分；后面三部分必须独立描述本月动态和相关行业，不要重复给出配置建议。
...
5. 配置建议只能集中写在第一部分，后面三部分主要是帮助用户理解近一个月动态
6. 后面三部分要补上与候选配置相关的行业内容，优先关注宽基、红利、黄金、有色、科技、机器人、消费、资源等方向
7. 第二到第四部分应吸收行业研究方法：不仅写宏观事件本身，还要写行业趋势、受影响较大的板块或龙头类型、市场动态和后续观察点
8. 第一部分必须建立在本月调研结论之上：先综合第二到第四部分的研究发现，再回到第一部分给出配置与操作建议

## 输出要求

只输出最终月报正文，使用自然语言中文，必须包含以下 4 个区块标题，且顺序固定：
...
- 第一部分不能只根据持仓快照机械给建议，必须明显吸收本月宏观、行业和大类资产调研结果
- 第一部分应回答“本月研究结论对当前配置意味着什么”，而不是孤立地重复仓位现状
...
- 第二到第四部分的重点是“帮助用户理解这一个月行业发生了什么”，而不是给出持仓操作指令
...
===== WEEKLY MATERIAL PACKET JSON START =====
{
  ...
  "portfolio_snapshot": {
    "generated_at": "2026-03-08T10:10:40",
    "portfolio": {
      "position_count": 4,
