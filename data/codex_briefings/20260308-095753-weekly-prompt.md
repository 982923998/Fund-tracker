...
第一部分必须体现本周宏观、行业和大类资产研究结果对当前配置的含义，不能只重复仓位现状。
...
8. 第一部分必须建立在本周调研结论之上：先综合第二到第四部分的研究发现，再回到第一部分给出配置与操作建议
...
- 第一部分不能只根据持仓快照机械给建议，必须明显吸收本周宏观、行业和大类资产调研结果
- 第一部分应回答“本周研究结论对当前配置意味着什么”，而不是孤立地重复仓位现状
...
===== WEEKLY MATERIAL PACKET JSON START =====
{
  ...
  "portfolio_snapshot": {
    "generated_at": "2026-03-08T09:57:08",
    "portfolio": {
      "position_count": 4,
